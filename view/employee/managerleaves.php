    ...
    <button><a href ="employee_page.php">Back to Home</a></button>
        <table class="table">
          ...
          $query = "SELECT e.id,e.firstName,e.email,e1.lid,e1.leavetype,e1.reason,e1.fromdate,e1.todate,e1.status,e1.adminremarks FROM tblemployees e, tblapplyleave e1 WHERE 
           e.id IN (SELECT employee FROM manageremployee WHERE manager='$id') AND e.email = e1.email";
            ...
            { 
                ...
                <tr>
                  <th scope="row">'.$i.'</th>
                    ...
                    <td><button><a href="manager_action.php?actionid='.$leaveid.' & managerid='.$id.'">'.$button.'</a></button></td>
                </tr>';
                $i++;
                }        
            }
